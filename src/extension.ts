 ...
 * A standardized return type for AI actions, indicating success or failure along with relevant data.
 * @property {boolean} success - True if the action was successful, false otherwise.
 * @property {string} [content] - The content generated by the AI if the action was successful.
 ...
 * Useful for introducing delays in async workflows.
...
/**
 * Handles the "explain selection" command by retrieving the selected code,
 * constructing an AI prompt, interacting with the AI service, and cleaning the output.
 * It ensures an active editor and selection are present, checks for API key and model,
 * and then sends the request to the AI, returning a structured result.
 * @param {SidebarProvider} sidebarProvider - The instance of the `SidebarProvider` to access AI services and API key management.
 * @returns {Promise<ActionResult>} A Promise that resolves to an `ActionResult` indicating success or failure
 *                                  and containing the explanation content or an error message.
 */
...
/**
 * The primary entry point for the Minovative Mind VS Code extension.
 * This asynchronous function is called when the extension is activated.
 *
 * It performs the following key tasks:
 * 1. Initializes the `SidebarProvider` to manage the extension's webview and AI services,
 *    determining the workspace root URI for context.
 * 2. Awaits the `sidebarProvider`'s initialization to ensure API keys and settings are loaded.
 * 3. Registers the `SidebarProvider` as a `WebviewViewProvider` with VS Code.
 * 4. Registers the core commands:
 *    - `minovative-mind.modifySelection`: Handles AI-driven code modifications.
 *      - Gathers contextual information (selection, full file, language, diagnostics, symbols).
 *      - Prompts the user to select an action (`/fix`, `/merge`, `chat`, `custom prompt`).
 *      - Gathers additional user input for chat/custom prompts.
 *      - Implements contextual code selection logic: uses explicit selection if available,
 *        otherwise attempts to auto-select relevant code based on command (e.g., errors for `/fix`, full file for `/merge`).
 *      - Visually updates the editor's selection if auto-selection occurred.
 *      - Formats diagnostic and merge conflict information as context.
 *      - Composes a detailed prompt message based on the selected action and gathered context.
 *      - Prefills the composed message into the sidebar chat input and focuses the sidebar.
 *    - `minovative-mind.explainSelection`: Handles AI-driven code explanations.
 *      - Uses the `executeExplainAction` helper function to get an explanation.
 *      - Displays the explanation in a VS Code information modal or an error message.
 *      - Shows progress notifications during the explanation process.
 *    - `minovative-mind.activitybar.focus`: A utility command to programmatically focus the Minovative Mind sidebar.
 *
 * @param {vscode.ExtensionContext} context - The context object provided by VS Code,
 *                                            used for managing extension subscriptions, storage, and URIs.
 */
...
/**
 * Performs cleanup tasks when the Minovative Mind extension is deactivated.
 * This includes resetting the AI client to ensure no lingering connections or states.
 */
