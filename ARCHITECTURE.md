...
- **Key Features**:
  - **AI Prompt Engineering (`src/context/smartContextSelector.ts`)**: The AI-driven file selection mechanism is enhanced to focus on deeper semantic and functional relevance. It specifically enforces a **Relationship-First Strategy**, prioritizing structural relationships (definitions, references, implementations) over generic text searches. It now features **AI-driven Request Categorization**, where user requests are classified (`BUG_FIX`, `NEW_FEATURE`, `REFACTOR`, etc.) to dynamically tailor investigation instructions and prioritize relevant diagnostic or symbol information.
  - **Optimized Project Structure (Discovery View)**: For projects exceeding a specific threshold, the selector generates a high-level **Discovery View** containing only top-level directories and essential root configuration files. This forces the Context Agent to use specialized tools for finding specific files, reducing token usage by up to 90%.
...
- **Responsibility**: Enables the AI to actively explore and investigate the codebase using safe terminal commands and deep IDE symbol tools during the context gathering phase, rather than relying solely on static file lists and summaries. Ensures the downstream AI receives complete, actionable context with zero gaps.
- **Key Features**:
  ...
  - **Deep Intent Decomposition**: Before investigation begins, `intentDecomposer.ts` breaks user requests into structured sub-intents, expanded search keywords, and domain context. Handles vague requests by generating multiple interpretation branches.
  - **Conversation Intelligence**: `chatHistoryAnalyzer.ts` mines chat history using **AI-powered entity extraction** (`CONVERSATION_ENTITY_EXTRACTION_TOOL`, `IMPLICIT_ASSUMPTION_DETECTION_TOOL`) to identify recurring topics, recently discussed files, code identifiers, and implicit assumptions.
  - **Cross-File Relationship Inference**: `crossFileInference.ts` discovers indirectly related files through 5 rules-based strategies (test pairing, naming siblings, feature folders, barrel inclusion, keyword matching) augmented by **AI-powered file name classification** for `coreName` and `architecturalRole` detection.
  - **Zero Deferred Research (Rule #1)**: The agent's most critical rule: the downstream AI cannot do its own research. The agent must never finish with gaps that would force the AI to say "I need to look at..." Finishing with known gaps is a critical failure.
  - **Confidence-Gated Validation**: A mandatory `validate_selection` self-assessment requires confidence ≥ 8/10 before `finish_selection` is allowed. If the agent reports `missingAreas` but attempts to finish without investigating them, it is automatically bounced back, validation state is reset, and it must re-investigate and re-validate.
  - **In-File Context Clue Tracking (Rule #13)**: The agent actively scans file content for textual references not captured by import analysis — comments (`// See also:`), string literals/routes, TODO/FIXME, error messages, config keys, and type assertions — and follows them to related files.
  ...
  - **Transparent Logging & UI**: Every command executed by the Context Agent is logged to the chat interface with terminal-like styling (transparent backgrounds). Logs feature **collapsible code blocks** (Show Code/Hide Code toggle), are **expanded by default** with `max-height: 200px` and auto-scrolling, to keep detailed output visible and compact.
  - **Progressive Context Discovery**: For larger projects (>10 files), the agent receives a truncated "Optimized View" of the project structure and discovers files on-demand, reducing token usage by up to 90%. This "Discovery View" emphasizes structural exploration over linear file listing.
  - **Intent-Aware Investigation**: The agent automatically detects user intent (e.g., bug fixing vs. documentation) and receives tailored instructions that prioritize relevant data (e.g., diagnostics for bug fixes, symbol hierarchies for feature additions).
- **Key Files**: `src/context/smartContextSelector.ts`, `src/context/safeCommandExecutor.ts`, `src/context/intentDecomposer.ts`, `src/context/chatHistoryAnalyzer.ts`, `src/context/crossFileInference.ts`, `src/services/lightweightClassificationService.ts`
...
- **Skip Plan Confirmation**: The "Fast Forward" toggle (`skipPlanConfirmation` setting) allows `PlanService.handleInitialPlanRequest` to bypass the user confirmation step entirely. When enabled, the plan explanation is streamed, and execution begins automatically without waiting for manual approval.
...
- **Inline Error Recovery**: Ambiguous match (`AmbiguousMatchError`) and missing search block (`SearchBlockNotFoundError`) errors during `modify_file` steps are now retried autonomously within the execution loop. Clarification context (including symbol locations for ambiguity) is accumulated and appended to subsequent AI retry prompts, eliminating separate recursive calls.
