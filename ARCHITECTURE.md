...
- **Responsibility**: Enables the AI to actively explore and investigate the codebase using precise "Vision" tools and safe terminal commands during the context gathering phase, rather than relying solely on static file lists and summaries.
- **Key Features**:
  - **Reasoning Loop**: The `selectRelevantFilesAI` function in `smartContextSelector.ts` implements a multi-turn agentic loop. The AI (utilizing **Gemini Flash Lite**) is given a suite of semantic and system tools and can iteratively run commands, observe outputs, and refine its understanding before making a final file selection with `finish_selection`.
  - **Precise Vision Tools**: The agent utilizes advanced semantic tools for structural discovery and navigation:
    - `get_file_outline`: Retrieves a structured map of classes, methods, and variables.
    - `get_symbol_definitions` & `get_symbol_implementations`: Enables "Semantic Teleportation" to exact code locations.
    - `find_symbol_references`: Traces symbol usage across the entire workspace.
    - `get_diagnostics`: Pulls real-time VS Code errors/warnings into the investigation context.
  - **Safe Command Execution**: The `SafeCommandExecutor` class (`src/context/safeCommandExecutor.ts`) enforces strict security by allowlisting only read-only commands (`ls`, `grep`, `find`, `git grep`, `wc`, `file`) and blocking dangerous operations or those that encourage low-precision "scanning" (like `cat`, `head`, `tail`, `sed`).
  - **Surgical Gatekeeper**: To eliminate token bloat and maximize precision, the system implements a hard gatekeeper in the `finish_selection` tool. If the AI attempts to select a full file larger than 100 lines, the call is rejected with a `PRECISION ERROR`, forcing the agent to provide specific line ranges (e.g., `file.ts:20-50`).
