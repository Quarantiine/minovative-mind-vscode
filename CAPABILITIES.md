...
- **Project Context & Intelligence**: Deep codebase analysis, Symbol Intelligence, **Progressive Discovery** for large projects, and **Relationship-First** context strategy.
...
- **Output Validation & Precision**: Employs sophisticated heuristics and **AI Integrity Validation** to sanitize and validate AI-generated code.
- **Surgical Code Editing & Structured Extraction**: Uses an anchored search/replace block system (`SEARC#H / ===#=== / REPLAC#E`) to update specific file segments. The system now utilizes AI function calling (`ENTITY_EXTRACTION_TOOL`) to perform granular entity analysis, ensuring that change summaries accurately reflect modifications to functions, classes, and variables.
...
- **Enhanced User Experience**: Provides immediate visual feedback with a **Smart Flash** effect that highlights changes across the full line width. The chat interface now displays polished, **Narrative Change Summaries** that describe modifications in human-readable sentences, making it easier to track complex plans.
...
- **Prioritized Context**: Prioritizes recently modified files, symbol links, and uncommitted git changes.
- **Structured AI File Analysis**: Every file in the sequential context is analyzed using **AI Function Calling** (`FILE_ANALYSIS_TOOL`), producing structured metadata including complexity estimation, main purpose detection, and semantic insights.
- **Intelligent Dependency Extraction**: Uses AI tools (`DEPENDENCY_EXTRACTION_TOOL`) to build a reliable map of project-wide relationships, ensuring the AI model understands the functional architecture of your code.
- **Cached Analysis**: Caches file analysis results to improve performance for symbol and reference lookups.
- **Smart Discovery View**: For large projects, the system generates a high-level **Discovery View** (optimized project structure) that forces the Context Agent to use specialized tools for finding specific files, drastically reducing token consumption and noise.
...
- **Progressive Discovery**: For large repositories, the agent starts with a highly efficient, truncated view of the project structure and discovers files on-demand using **Progressive Discovery**, reducing initial context token usage by up to 90%. It prioritizes a **Relationship-First Strategy**, leveraging deep structural ties (definitions, references, implementations) to build an accurate mental model faster.
- **AI-Driven Request Categorization**: Automatically classifies the user's intent into categories like `BUG_FIX`, `NEW_FEATURE`, `REFACTOR`, or `DOCUMENTATION`. This classification is used to dynamically generate tailored investigation instructions, ensuring the agent focuses on the most relevant data (e.g., diagnostics for bugs, extension points for new features).
- **Deep Intent Decomposition**: Breaks user requests into structured sub-intents, expanded search keywords, and domain context before file investigation begins. Handles vague requests by generating multiple interpretation branches.
- **Conversation Intelligence**: Mines chat history using **AI-powered entity extraction** to identify recurring topics, recently discussed files, code identifiers, and implicit assumptions — replacing raw truncated history with structured analysis.
- **Cross-File Relationship Inference**: Discovers indirectly related files through 5 rules-based strategies (test pairing, naming siblings, feature folders, barrel inclusion, keyword matching) augmented by **AI-powered file name classification** for accurate `coreName` and `architecturalRole` detection.
- **Zero Deferred Research Enforcement**: The agent's #1 rule is that the downstream AI cannot do its own research. The agent must never finish with gaps that would force the AI to say "I need to look at..." — finishing with known gaps is a critical failure regardless of confidence score.
- **Confidence-Gated Validation**: A mandatory `validate_selection` self-assessment requires confidence ≥ 8/10 before `finish_selection` is allowed. If the agent reports `missingAreas` but attempts to finish without investigating them, it is automatically bounced back and must re-validate.
- **In-File Context Clue Tracking**: The agent actively scans file content for textual references not captured by import analysis — comments, string literals, TODO/FIXME, error messages, configuration keys, and type assertions — and follows them to related files.
- **AI-Driven Error Investigation**: Automatically detects when you're asking about errors or bugs and proactively investigates error messages, stack traces, and relevant code paths using categorizations and real-time diagnostics before generating a response.
