...
- **Code Structure Information**: Document Symbols (functions, classes, types), code references, and other structural data derived from your codebase.
...
- **File Metadata**: File names, paths, sizes, and modification times are analyzed to support features like **Progressive Discovery**, enabling the agent to navigate large codebases efficiently without reading full file contents.
...
- **Self-Correction Feedback**: During autonomous self-correction cycles, the AI analyzes its own previous output and any resulting new diagnostics to generate improved code. This "feedback loop" data is processed iteratively.

### 3.5 Usage Telemetry (Strictly Local)

To help you monitor costs and performance, the extension tracks specific usage metrics locally. This includes:

- **Token Counts**: Input and output token usage for each request.
- **Request Success/Failure Rates**: Tracking of successful vs. failed API calls to monitor system reliability.
- **Model Usage**: Which specific Gemini models (e.g., Flash Lite vs. Pro) are being utilized.

**Note**: This specific telemetry data is stored _only_ on your local machine and disaggregated. It is NOT transmitted to Ward Innovations DBA.
...
- **Token Consumption Statistics**: The extension precisely measures token consumption for all AI requests, including success and failure metrics. These statistics are tracked locally and displayed within the extension's UI for your transparency and cost monitoring. This data is not transmitted to Ward Innovations DBA' servers.
...
- **Contextual Feedback**: During "Self-Correction" workflows, the _output_ of previous steps (including potential hallucinations or errors) and the resulting error diagnostics are fed back into the model as context to enable the AI to learn from and fix its mistakes.
